...
- 2026-05-18 / WI-029: Mobile Playwright verification at 390x844 with real trip data passed the touch assignment checks and saved [WI-029-after-2.png](./screenshots/after/WI-029-after-2.png). Assign is 44x44 on mobile; desktop Assign and Move remain compact icon-only controls. `node tests/city-nav-regression.js` passed; `node tests/run-tests.js` still stops on the existing map harness issue `L is not defined`.
