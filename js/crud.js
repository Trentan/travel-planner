...
function openActivityModalUnified(legIdx, activityIdx = null, options = {}) {
  ...
  const updateDualTimeNotice = () => {
    ...
    const localTz = typeof getCityTimezone === 'function' ? getCityTimezone(leg?.label) : '';
    ...
    if (timeVal && localTz && homeTz && localTz !== homeTz && typeof formatDualTimeDisplay === 'function') {
      const dualText = formatDualTimeDisplay(timeVal, dateVal, localTz, homeTz);
      if (dualText) {
        noticeEl.textContent = `${dualText} (${localTz.split('/')[1] || localTz} local time)`;
