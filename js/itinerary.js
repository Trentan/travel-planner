...
<h4>💡 Tips</h4>
<ul class="tips-list">${(leg.legTips || []).map((t, i) => `<li class="tip-item"><span contenteditable="${isEditMode}" onblur="updateLegTip(${legIndex}, ${i}, this.innerText)">${t}</span><button class="del-btn" title="Delete Tip" onclick="deleteLegTip(${legIndex}, ${i})">×</button></li>`).join('')}</ul>
...
<button class="del-btn" title="Remove Journey" onclick="deleteJourney('${journey.id}'); buildItinerary();">×</button>
<span class="cost-item-text">${icon} ${journey.notes || journey.fromLocation + ' → ' + journey.toLocation}</span>
<div class="cost-item-actions">
<span class="status-badge" style="background:${statusColor}; ${isEditMode ? 'cursor:pointer;' : ''}" title="${isEditMode ? 'Click to toggle status' : 'Booking status'}" ${isEditMode ? `onclick="event.stopPropagation(); event.preventDefault(); toggleJourneyStatus('${journey.id}'); buildItinerary(); return false;"` : ''}>${statusIcon} ${status === 'booked' ? 'Booked' : 'Planned'}</span>
${showRef ? `<input type="text" class="booking-ref-input confirmed" value="${journey.bookingReference || ''}" placeholder="Ref #" onchange="updateJourneyBookingRef('${journey.id}', this.value); buildItinerary();" ${isEditMode ? '' : 'readonly'}/>` : ''}
<span class="budget-field">$<span contenteditable="${isEditMode}" onblur="updateJourneyCost('${journey.id}', this.innerText); buildItinerary();">${journey.cost || '0'}</span></span>
</div>
</div>`;
}).join('')}
</div>${isEditMode ? `<button class="add-btn" onclick="openAddJourneyModal()">+ Add Journey</button>` : ''}
</div>
