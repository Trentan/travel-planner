...
Your job is to conduct a complete polish audit of this PWA across **two view modes**,
...
- **Stack:** Vanilla JS, `index.html`, `/js/*.js` modules, Tailwind css, service worker
- **Fonts:** ???
...
|---------|---------|----------|-------------------------------------------------------------------------------------------|
| `DDE` | Desktop | 1440 × 900 | Default (expanded cards, full labels)                                                     |
| `MDE` | Mobile  | 390 × 844 | Default (enhanced for mobile device use / smaller screens, swipe actions, button touches) |
...
For each mode prefix (`dde-`, `mde-`, ), note the highest numbered file.
...
|----|-------|----------|-------|
| DDE | Desktop | 1440×900 | Default density |
| MDE | Mobile | 390×844 | Default density |
...
6. Take an after screenshot: `node polish/scripts/take-screenshots.js --wi WI-XXX --mode [DDE|MDE]`
...
- Accept an optional `--mode [DDE|MDE]` flag to target a single mode
...
**Issues found:**
① [name] — [element · location · what fails · which modes affected: DDE / MDE ]
② ...
...
- For every issue, explicitly state which of the four modes (DDE / MDE ) it affects.
...
3. Capture all missing before screenshots across all four modes: DDE (Desktop Detailed), MDE (Mobile Detailed). Skip any screenshot file that already exists.
...
Complete it end-to-end: implement the fix, verify in modes [DDE / MDE ],
